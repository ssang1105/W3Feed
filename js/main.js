...
for(var i=1; i<=maxLength; i++){
	...
	$(document).delegate('#select-choice', 'change', function(e) {
		...
		if(selected == 'politics'){
			getOnlineFeed('https://pipes.yahoo.com/pipes/pipe.run?_id=7309ac972f84d6d6fd3d34d913aa03f1&_render=rss');
			console.log('aa')
				getOnlineFeed('https://pipes.yahoo.com/pipes/pipe.run?_id=7309ac972f84d6d6fd3d34d913aa03f1&_render=rss');
				$('#articleList').html($('#articleList').html());
		}
		else if(selected == 'economy'){
			getOnlineFeed('https://pipes.yahoo.com/pipes/pipe.run?_id=a4e96a840ec9fa0596b08554873df208&_render=rss');
			$('#articleList').html($('#articleList').html());
			var listEntries = function(json) {
				  if (!json.responseData.feed.entries) return false;
				  $('#widgetTitle').text(json.responseData.feed.title);
			...
			getOnlineFeed('https://pipes.yahoo.com/pipes/pipe.run?_id=ba7008c6400b4a21930d8b02e9c26211&_render=rss');
			$('#articleList').html($('#articleList').html());
			var listEntries = function(json) {
				  if (!json.responseData.feed.entries) return false;
				  $('#widgetTitle').text(json.responseData.feed.title);
			...
			getOnlineFeed('https://pipes.yahoo.com/pipes/pipe.run?_id=4c5a0c1d7f38cb11b81fd8382c53f14e&_render=rss');
			$('#articleList').html($('#articleList').html());
			var listEntries = function(json) {
				  if (!json.responseData.feed.entries) return false;
				  $('#widgetTitle').text(json.responseData.feed.title);
